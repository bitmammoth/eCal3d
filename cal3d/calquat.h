...
public:
	...
	inline CalQuaternion(const CalMatrix& m) { *this = m; }
...
public:
	...
	inline void operator=(const CalMatrix & m)
	{
  		...
  		if (T > 0.0001) {
    			...
    			z = ( m.dxdy - m.dydx ) / S;
    			w = 0.25 * S;
  		} else if ( m.dxdx > m.dydy && m.dxdx > m.dzdz )  {  // Column 0:
    			double S = sqrt( 1.0 + m.dxdx - m.dydy - m.dzdz ) * 2;
    			x = 0.25 * S;
    			y = (m.dxdy + m.dydx ) / S;
    			z = (m.dzdx + m.dxdz ) / S;
    			w = (m.dydz - m.dzdy ) / S;
  		} else if ( m.dydy > m.dzdz ) {                      // Column 1:
    			double S = sqrt( 1.0 + m.dydy - m.dxdx - m.dzdz ) * 2;
    			x = (m.dxdy + m.dydx ) / S;
    			y = 0.25 * S;
    			z = (m.dydz + m.dzdy ) / S;
    			w = (m.dzdx - m.dxdz ) / S;
  		} else {                                             // Column 2:
    			double S = sqrt( 1.0 + m.dzdz - m.dxdx - m.dydy ) * 2;
    			x = (m.dzdx + m.dxdz ) / S;
    			y = (m.dydz + m.dzdy ) / S;
    			z = 0.25 * S;
    			w = (m.dxdy - m.dydx ) / S;
...
*/	
	...
	inline void blendRadians(float rad, const CalQuaternion& q)
	{
		...
		if (rad < 0.0) rad = -rad;
		double angw = w * q.w + x * q.x + y * q.y + z * q.z;
		if (angw < 0.0) angw = -angw;
		double angle = (angw > 0.99999) ? 0.0 : acos(angw);
		if (rad >= angle) { x=q.x; y=q.y; z=q.z; w=q.w; }
		else blend(rad / angle,q);
	}
